...
//  --> Defines Compiler version to use for Solidity
...
//  --> not compatible with compiler version -> ERROR
...
//  --> e.g. ^2.3.4 includes all available versions in 2.#.#
//  --> ~2.3.4 includes all versions 2.3.#
...
let contractAddress2 = "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512";

let contractName3 = "Lock3";
...
async function readVar() {
    ...
    let lock = await hre.ethers.getContractAt(contractName2, contractAddress2, firstDefSigner);
    // console.log("Contract Address:", lock.address);

    let name = await lock.name();
    ...
    // Your code here!
    let unixTime = Number(await lock.unlockTime());
...
async function getContractAndSigner(cName, cAddress, signerIdx = 0) {
    ...
    // Your code here!
    let DefSigners = await hre.ethers.getSigners();
    let singleSigner = DefSigners[signerIdx];

    let contract = await hre.ethers.getContractAt(cName, cAddress, singleSigner);

    return [singleSigner, contract];
}
/*
getContractAndSigner(contractName, contractAddress).then(function([singleSigner, contract]){
    console.log("Signer:", singleSigner);
...
async function constructor() {
    ...
    // Your code here!
    const [mySigner, myContract] = await getContractAndSigner(contractName3, contractAddress3);
    console.log("Blocknumber:", Number(await myContract.blockNumber()));
}
...
async function events() {
    ...
    myContract.on("Withdrawal", (amount) => {
        console.log("Amount withdrawn:", ethers.utils.formatEther(amount));
    });
    myContract.on("WithdrawalAttempt", (myAdress) => {
        console.log("Address to withdraw to:", myAdress);
    });
